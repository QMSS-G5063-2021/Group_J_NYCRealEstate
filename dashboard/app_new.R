                 ...
                 tabPanel("Main",
                          mainPanel(
                            ...
                            "This project examines the Manhattan residential and commercial building construction permits 
                             ...
                             Community Survey, and neighborhood descriptions scraped from an archive of Wikipedia.
                             ", br(), br(),
                            
                            "To understand the trends in construction as well as the people and culture in NYC neighborhoods,
                              we have explored the data through the four tabs on this dashboard:", 
                            br(), br(),
                            
                            "1)", strong("Construction "),"explores the location and types of construction projects and the relationship between land 
                              use changes",
                            br(), br(),
                            
                            "2)", strong("Home Values "), "compares rents and home values to construction permits, and shows proportion 
                              of renter vs owner residents in neighborhoods",
                            br(), br(),
                            
                            "3)", strong("Demographics "), "does a deep-dive into the people in each of the 10 neighborhoods to understand median 
                            age, population, median income, and more",
                            br(), br(),
                            
                            "4)", strong("Neighorhoods in Words "), "examines how user contributed descriptions of Manhattan neighborhoods have changed 
                 ...
                 tabPanel("Construction",
                          mainPanel(
                            fluidRow(
                              ...
                              br(),
                              h3('Overview of Manhattan Construction'),
                              p('Within the past 20 years, there have been many types of construction projects within Manhattan. Judging by the maps, some of the highest 
                                  ...
                                  buildings, without many empty sites on which to build. On the other hand, many of the building alteration projects have occurred within
                            ...
                            fluidRow(
                              h3('Construction Project Composition by Neighborhood'),
                            ...
                            fluidRow(
                              h3('Building Occupancy Transformations'),
                 ...
                 tabPanel("Home Value",
                          mainPanel(
                            fluidRow(
                              ...
                              br(),
                              p("The plots below show the number of new building permits and alteration permits.
                                  This can be compared with median gross rent and home value. A pattern emerges to
                                  show that neighborhoods with higher home prices and rents have higher number of permits. 
                                  Note that those neighborhoods are also likely to have higher proportion of residents who own 
                                  their homes.
                                  "),
...
server <- function(input, output) {
  ...
  output$renter_pct <- renderPlotly({
    data <- renter_pct %>%
      ...
      mutate(variable = ifelse(variable == "renter_pct", "Renter",
                               ifelse(variable == "owner_pct", "Owner", variable)))
    
    p <- ggplot(data, aes(year, estimate, fill = variable)) +
      geom_bar(stat = "identity", position = "fill") + 
      facet_wrap(~puma_name) +
      theme_minimal() +
      scale_fill_manual(values = c("Owner" = "gray", "Renter" = "#1B9E77"), 
                        name = "Occupant") +
      scale_x_continuous(breaks= c(2009:2019)) +
    ...
    style(p, text = paste("Owner Proportion: ",round(data[data$variable == "Owner", ]$estimate, digits = 3),"\n",
                          "Renter Proportion: ",round(data[data$variable == "Renter", ]$estimate, digits = 3)))
  })
  ...
  output$median_age <- renderPlot({
    ...
    # 
    # ggplotly(p)
    # style(p, text = paste(mean_group$puma_name,"\n",
    #                       'Value: ',mean_group$median_age))
    # 
  }) 
